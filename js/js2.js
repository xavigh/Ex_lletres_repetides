...
function nameLetters(){
    ...
    for(i=0; i < name.length; i++){// first we check if it is a digit or a letter

        if( isNaN(name[i]) ){  //if yes it is a number then check if it is a vocal
              var str="aeiou_AEIOU";
              if(  str.match(name[i]) ){
    ...
    }//for loop // if itś a digit then we input the digit.
}// end of function
